...
Using the mechanisms described in {{?RFC7050}} or {{?I-D.draft-hunek-v6ops-nat64-srv}} may not function because these need a resolver to work.
...
Since the iterative resolver will be used inside an IPv6-only network, the server can also perform DNS64 {{!DNS64=RFC6147}} when an AAAA record is queried from a STUB resolver but the domain only has an A record.
